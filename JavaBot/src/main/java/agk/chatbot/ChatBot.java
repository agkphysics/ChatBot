...
public class ChatBot implements StandardCBRApplication {
    ...
    public ChatBot(Path p) throws FileNotFoundException {
        if (p != null && Files.exists(p)) {
            ...
            for (String loc : locations) {
                ...
                if (xmlURL != null) {
                    try {
                        Path xmlPath = Paths.get(xmlURL.toURI());
        ...
        System.out.println("Started bot application -- CBR Chat Bot version " + VERSION);
        System.out.println("--------------------------------------------------");
    }
    ...
    public void cycle(CBRQuery query) throws ExecutionException {
        ...
        for (NLPToken t : statement.getText().getAllTokens()) {
            if (t.getRawContent().toLowerCase().contains("bye")) {
                finished = true;
        ...
        if (topEval < MIN_SIMILARITY) {
            ...
            for (RetrievalResult rr : topRes) {
                if (rr.get_case().getSolution() != null) {
                    response = (ChatResponse)rr.get_case().getSolution();
                    break;
                }
            }
            if (response == null) response = getConfusedResponse(statement);
            currentThread.add(statement);
            currentThread.add(response);
        }
