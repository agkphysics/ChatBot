...
public class ProcessedXMLConnector implements Connector {
    ...
    public static Collection<CBRCase> convertFileToCases(File file) {
        ...
        try {
            ...
            for (int i = 0; i < utters.getLength(); i++) {
                ...
                for (NLPSentence s : txt.getSentences()) sentences.add(s.getText());
                utterance = String.join(" ", sentences);
    ...
    public Collection<CBRCase> retrieveAllCases() {
        final Collection<CBRCase> coll = Collections.synchronizedCollection(new ArrayList<CBRCase>());
